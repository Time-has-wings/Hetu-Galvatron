...
class TimeCostModelBase:
    ...
    def estimate_dp_communication_time(self):
        ...
        if self.sp_size != 1: # deepspeed-ulysses
    ...
    def estimate_tp_communication_time(self):
        ...
        if args.sp_space == 'tp+sp':
            ...
            if self.tp_sp_size == 1:
                ...
                self.per_tp_message_size = self.lbsz * self.seq_length * self.hidden_size * (2 if args.mixed_precision else 4)
                if  self.per_tp_message_size in self.tp_sp_dict:
                    per_tp_message_time = self.tp_sp_dict[ self.per_tp_message_size]
                else:
                    def linear_func(x, m, c):
                        return m * x + c
                    per_tp_message_time = linear_func(1 / 1024 / 1024 *  self.per_tp_message_size, *self.tp_sp_dict["popt"])
            ...
            tp_comm_times = 4 
            self.tp_message_size = 2 * (self.tp_sp_size - 1) / self.tp_sp_size * (self.lbsz * self.seq_length * self.hidden_size * tp_comm_times * 4 / 1024 / 1024) * self.layer_num
            if self.checkpoint:
            ...
            if self.sp_size != 1: # deepspeed-ulysses
                tc = args.comm_coe_dict['%d'%self.sp_size] if '%d'%self.sp_size in args.comm_coe_dict.keys() else args.comm_coe_dict['%d_1'%self.sp_size]
            else:
                if self.tp_size == 1 or self.dp_size == 1:
    ...
    def get_result(self, no_gradient_sync:bool = False):
        ...
        if self.pp_size >= 1:
            if self.tp_sp_size == 1 and self.dp_size > 1: # pp+dp
                ...
                if self.tp_sp_size < self.tp_sp_size * self.dp_size // 2:
                    overlap_part, rest_part, _ = self.bct_dp_overlap(self.dp_message_size * factor, self.bct)
                    overall_overhead = self.fct + overlap_part + rest_part + self.tp_communication_time + args.extra_overhead
                    result = overall_overhead
                else:
                    overlap_part, rest_part, _ = self.bct_dp_overlap(self.dp_message_size * factor, self.bct * 1 / 2)
